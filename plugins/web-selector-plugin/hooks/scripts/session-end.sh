#!/usr/bin/env bash
# SessionEnd hook: deregister this session from the bridge server.
...
# Deregister — silently ignore if bridge is already down
curl -sf -X POST "${BRIDGE_URL}/session/deregister" \
