...
- `meta.authoritativeRenderer` — `remotion` (default) | `final-cut-pro`. Which film the user is
  approving. A Remotion draft renders no Motion template, so with `final-cut-pro` the review
  gate needs the film exported from FCP.
- `meta.videoId` — `NN-slug` (`01-rex-overview`) when a repository holds more than one video.
  Delivery outputs go under `demo/<videoId>/`. Numbers are stable and never reused.
- `meta.voice` — the narrator contract: `provider`, `voiceId`, `modelId`, `speed`, `stability`,
  `similarityBoost`, plus anything else regeneration must reproduce (`style`, `language`,
  `use_speaker_boost`, pronunciation overrides). Free-form — record every parameter you set,
  and never a credential. A display name alone does not find the voice again.
...
`demo/out/probe.fcpxml` (`--probe`) is the same document cut to about ten seconds, carrying
every named template with a sentinel in each text layer. Import it before the film: a
DTD-valid file can still import with warnings.

## demo/out/fcp-finish-manifest.json

Written by `fcp-finish-manifest.mjs`. What the finish is made of: every input and output
hashed, every template with its length, text layers, drop zones and how it can be moved, and
`byHand` — what FCPXML cannot carry. `review.status` stays `pending-human-watch-listen` until
a person has watched and listened. It is the answer to "what is in this video" six months
later, and to "what will I lose if I re-export".

## demo/<videoId>/youtube/

Written by `youtube-package.mjs` after the user approves the film: `package.json` (title,
description, chapters from the **measured** cut, provenance, checks) and a copy-ready
`README.md`. A second run keeps hand edits while the movie's hash is unchanged. It never
...
Commit: `brief.md`, `inventory.json`, `storyboard.json`, `prep/` (except `prep/state/`), any
customisation of `studio/src`, and — once a film is approved — `<videoId>/final/` and
`<videoId>/youtube/`. Everything else — clips, raw takes, narration, `timeline.json`, renders,
the FCPXML, isolated app state — is regenerable and gitignored by `init-demo.sh`.

The approved delivery is the FCP **library**, not the FCPXML: a logo assigned to a template's
media well does not survive an XML export. Keep the library, the approved movie, and the
manifest that hashes both.
